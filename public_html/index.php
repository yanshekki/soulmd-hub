...
<div class="max-w-7xl w-full mx-auto px-4 sm:px-6 pb-20 pt-8">
    ...
    <div class="text-center py-12 md:py-20 relative">
        ...
        <a href="<?= url('/upgrade') ?>" class="inline-flex items-center gap-2 bg-gradient-to-r from-emerald-900/40 to-teal-900/40 text-emerald-300 border border-emerald-500/30 px-5 py-2 rounded-full text-sm font-medium mb-8 hover:scale-105 transition transform shadow-lg shadow-emerald-500/10">
            <i class="fas fa-rocket text-amber-400"></i> <?= __('Banner Text') ?>
        </a>
        
        <h1 class="text-5xl md:text-7xl font-extrabold tracking-tighter leading-tight mb-6 text-white">
            <?= __('Hero Title 1') ?><span class="gradient-text"><?= __('Hero Title 2') ?></span><?= __('Hero Title 3') ?>
        </h1>
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center items-center max-w-sm sm:max-w-none mx-auto">
            <a href="<?= url('/browse') ?>" class="w-full sm:w-auto flex items-center justify-center gap-3 px-10 py-4 bg-emerald-500 text-zinc-950 text-lg font-bold rounded-2xl hover:bg-emerald-400 transition shadow-xl shadow-emerald-500/20 hover:-translate-y-1">
                <i class="fas fa-compass"></i> <?= __('Discover Souls') ?>
            </a>
            <a href="<?= url('/generate') ?>" class="w-full sm:w-auto flex items-center justify-center gap-3 px-10 py-4 border border-white/20 bg-zinc-900/50 text-white text-lg font-bold rounded-2xl hover:bg-white/10 hover:border-white/40 transition backdrop-blur-sm">
                <i class="fas fa-magic"></i> <?= __('AI Generator') ?>
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 text-center mb-24 border-y border-white/5 py-10 bg-zinc-900/20">
        <div>
            ...
            <div class="text-emerald-400 text-sm font-bold uppercase tracking-widest mt-1"><?= __('Souls Shared') ?></div>
        </div>
        <div>
            <div class="text-4xl font-black text-white tracking-tight"><?= number_format($statsUsers) ?>+</div>
            <div class="text-emerald-400 text-sm font-bold uppercase tracking-widest mt-1"><?= __('Active Creators') ?></div>
        </div>
        <div>
            <div class="text-4xl font-black text-white tracking-tight"><?= number_format($statsTags) ?></div>
            <div class="text-emerald-400 text-sm font-bold uppercase tracking-widest mt-1"><?= __('Knowledge Domains') ?></div>
        </div>
    </div>
    ...
    <div class="mb-24">
        <div class="text-center mb-12">
            ...
            <p class="text-zinc-400"><?= __('Upgrade Subtext') ?></p>
        </div>
    ...
    <div class="mb-24">
        <div class="flex items-end justify-between mb-8">
            <div>
                <h2 class="text-2xl font-bold"><?= __('Popular Domains') ?></h2>
                <p class="text-zinc-400 text-sm mt-1"><?= __('Popular Domains Subtext') ?></p>
            </div>
            <a href="<?= url('/browse') ?>" class="text-emerald-400 text-sm font-medium hover:underline hidden sm:block"><?= __('Explore All') ?></a>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <?php foreach ($categories as $cat): ?>
                <a href="<?= url('/browse?role=' . urlencode($cat['slug'])) ?>" class="bg-zinc-900/60 hover:bg-zinc-800 transition p-6 rounded-3xl text-center shadow-lg border border-white/5 hover:border-emerald-500/20 group">
