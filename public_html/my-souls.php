...
// ==========================================
// 1. AJAX 取得單一 Soul 數據 (Edit 預填使用)
// 必須放在 setSEO 之前，確保回傳純淨的 JSON
// ==========================================
if (isset($_POST['ajax_get'])) {
    $id = (int)$_POST['id'];
    
    // 嚴格檢查：只允許撈出屬於自己的 Soul
...
// ==========================================
// 3. AJAX 編輯 Soul (Edit & Save Version)
// ==========================================
...
// 在所有 AJAX 判斷結束後，才輸出 HTML Header 內容
setSEO('My Souls', 'Manage your uploaded AI souls.');

// 撈出所有分類 (Role)，供編輯彈窗的 Select 選擇
...
<body class="bg-zinc-950 text-white min-h-screen flex flex-col">
    <nav class="max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 flex justify-between items-center">
        ...
        <div class="flex items-center gap-6 text-sm">
            ...
            <a href="upload" class="text-zinc-400 hover:text-white transition"><i class="fas fa-cloud-upload-alt"></i> Upload</a>
        </div>
    </nav>
    ...
    <script>
        ...
        async function editSoul(id) {
            ...
            try {
                // 使用 window.location.href 避免相對路徑的路由干擾
