...
<main class="max-w-5xl w-full mx-auto px-4 sm:px-6 pb-16 pt-8">
    <header class="text-center mb-12">
        <div class="inline-flex items-center gap-2 bg-emerald-900/20 text-emerald-400 px-4 py-1.5 rounded-full text-xs font-medium mb-6 border border-emerald-500/20">
            <i class="fas fa-layer-group" aria-hidden="true"></i> <?= __('Modular AI Generator') ?>
        </div>
    ...
    <section id="form-section" aria-label="AI Agent Generation Form">
        <div class="max-w-3xl mx-auto mb-8 flex flex-wrap justify-center gap-3">
            <span class="text-sm text-zinc-500 py-2" id="preset-label"><?= __('Quick Presets:') ?></span>
            <button type="button" aria-describedby="preset-label" onclick="fillTemplate('dev')" class="px-4 py-2 rounded-full bg-zinc-900 border border-white/10 text-sm hover:border-emerald-400/50 hover:text-emerald-400 transition"><?= __('Expert Coder') ?></button>
            <button type="button" aria-describedby="preset-label" onclick="fillTemplate('writer')" class="px-4 py-2 rounded-full bg-zinc-900 border border-white/10 text-sm hover:border-emerald-400/50 hover:text-emerald-400 transition"><?= __('Copywriter') ?></button>
            <button type="button" aria-describedby="preset-label" onclick="fillTemplate('assistant')" class="px-4 py-2 rounded-full bg-zinc-900 border border-white/10 text-sm hover:border-emerald-400/50 hover:text-emerald-400 transition"><?= __('Executive Assistant') ?></button>
        </div>
        ...
        <form id="generate-form" class="max-w-3xl mx-auto bg-zinc-900/50 border border-white/10 rounded-3xl p-8 backdrop-blur-sm shadow-2xl">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label for="input-role" class="block text-sm font-medium mb-2 text-zinc-300"><?= __('Role / Profession') ?></label>
                ...
                <div>
                    <label for="input-personality" class="block text-sm font-medium mb-2 text-zinc-300"><?= __('Personality Traits') ?></label>
            ...
            <div class="mb-6">
                <label for="input-expertise" class="block text-sm font-medium mb-2 text-zinc-300"><?= __('Expertise / Tech Stack') ?></label>
            ...
            <div class="mb-6">
                <label for="input-style" class="block text-sm font-medium mb-2 text-zinc-300"><?= __('Communication Style') ?></label>
            ...
            <div class="mb-8">
                <label for="input-special" class="block text-sm font-medium mb-2 text-zinc-300 flex justify-between">
            ...
            <button type="submit" id="submit-btn" aria-label="<?= __('Generate Modular Agent') ?>" class="w-full py-4 bg-emerald-500 text-zinc-950 text-lg font-bold rounded-2xl hover:bg-emerald-400 transition flex items-center justify-center gap-3 shadow-lg transform hover:-translate-y-0.5 duration-200">
                <span id="submit-text"><i class="fas fa-bolt mr-1" aria-hidden="true"></i> <?= __('Generate Modular Agent') ?></span>
    ...
    <section id="result-section" class="hidden animate-fade-in" aria-live="polite" aria-atomic="true">
        <div class="max-w-3xl mx-auto mb-6 flex justify-between items-end border-b border-white/10 pb-6">
            ...
            <div class="flex gap-3">
                <button type="button" onclick="resetForm()" aria-label="Start over" class="px-5 py-2.5 bg-zinc-800 border border-white/10 rounded-xl text-sm font-medium hover:bg-zinc-700 transition flex items-center gap-2 shadow">
                    <i class="fas fa-redo text-xs" aria-hidden="true"></i> <?= __('New') ?>
                </button>
            </div>
        </div>
        ...
        <div class="max-w-3xl mx-auto bg-zinc-900 border border-white/10 rounded-3xl p-6 flex flex-col mb-8 shadow-xl">
            <div class="flex justify-between items-center mb-4">
                <div class="font-bold text-lg flex items-center gap-2">
                    <i class="fas fa-folder-open text-purple-400" aria-hidden="true"></i> <?= __('JSON Output') ?>
                </div>
            </div>
            <pre id="folder-content" class="bg-zinc-950 border border-white/5 p-5 rounded-2xl text-[13px] whitespace-pre-wrap overflow-y-auto max-h-[450px] font-mono text-zinc-300 leading-relaxed shadow-inner" tabindex="0"></pre>
        </div>
        ...
        <div class="flex flex-col items-center justify-center pt-4">
            <a href="<?= url('/upload') ?>" class="px-12 py-4 bg-emerald-500 text-zinc-950 text-xl font-bold rounded-2xl hover:bg-emerald-400 transition flex items-center gap-3 shadow-lg hover:scale-105 transform duration-200">
                <?= __('Go to Upload') ?> <i class="fas fa-arrow-right" aria-hidden="true"></i>
            </a>
        </div>
    </section>
</main>
...
<script>
    ...
    genForm.addEventListener('submit', async function(e) {
        ...
        // Prompt Identity & Setup (JS replace)
