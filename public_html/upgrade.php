...
<div class="max-w-5xl w-full mx-auto px-4 sm:px-6 py-8 sm:py-12 flex-grow flex flex-col">
    <div class="text-center mb-10 sm:mb-16">
        ...
        <h1 class="text-3xl sm:text-4xl md:text-5xl font-extrabold tracking-tighter mb-4 leading-tight">Upgrade Your <span class="gradient-text">AI Brain</span></h1>
        ...
        <?php if ($isActivePremium): ?>
            ...
            <?php endif; ?>
            <div class="mt-5">
                <a href="/billing" class="text-xs text-emerald-400 hover:text-emerald-300 underline transition flex items-center justify-center gap-1.5">
                    <i class="fas fa-file-invoice-dollar"></i> View Billing History
                </a>
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 max-w-4xl mx-auto items-stretch mb-12">
        ...
        <div class="bg-zinc-900/60 border border-white/10 rounded-3xl p-6 sm:p-8 flex flex-col hover:border-emerald-500/30 transition-all duration-300 shadow-xl backdrop-blur-sm justify-between">
            <div>
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <div class="text-emerald-400 text-[10px] sm:text-xs font-bold tracking-widest uppercase mb-1">Standard Plan</div>
                        <h3 class="text-xl sm:text-2xl font-bold text-white tracking-tight">VIP Member</h3>
                    </div>
                    <?php if ($isVip): ?>
                ...
                <div class="text-4xl sm:text-5xl font-black text-white mb-3 tracking-tight">$<?= PRICE_VIP_MONTHLY ?> <span class="text-xs sm:text-sm text-zinc-500 font-normal tracking-normal">/ 30 Days</span></div>
                <p class="text-xs sm:text-sm text-zinc-400 mb-6 pb-6 border-b border-white/5 leading-relaxed">Perfect for daily general interactions, prompt engineering testing, and standard multi-file bot testing.</p>
                
                <ul class="space-y-4 mb-8 text-xs sm:text-sm text-zinc-300">
                    <li class="flex items-start gap-3"><i class="fas fa-check text-emerald-500 mt-0.5 shrink-0 w-4 text-center"></i> <span><b>Unlimited</b> turns per session</span></li>
                    <li class="flex items-start gap-3"><i class="fas fa-check text-emerald-500 mt-0.5 shrink-0 w-4 text-center"></i> <span>Up to <b><?= number_format(VIP_MAX_INPUT_CHARS) ?></b> chars input</span></li>
                    <li class="flex items-start gap-3"><i class="fas fa-check text-emerald-500 mt-0.5 shrink-0 w-4 text-center"></i> <span><b>Vision AI</b>: JPG / PNG analysis</span></li>
                    <li class="flex items-start gap-3"><i class="fas fa-check text-emerald-500 mt-0.5 shrink-0 w-4 text-center"></i> <span>Smart sliding memory retention</span></li>
                    <li class="flex items-start gap-3"><i class="fas fa-check text-emerald-500 mt-0.5 shrink-0 w-4 text-center"></i> <span>Private Mode switch lock</span></li>
                    <li class="flex items-start gap-3 opacity-40"><i class="fas fa-times text-zinc-600 mt-0.5 shrink-0 w-4 text-center"></i> <span class="line-through">Elite Reasoning Engine</span></li>
                </ul>
            </div>
            ...
            <div class="pt-4 border-t border-white/5 mt-auto">
                <?php if ($isVip || $isPro): ?>
                    <button disabled class="w-full py-3 bg-zinc-800/50 text-zinc-500 font-bold rounded-xl cursor-not-allowed border border-white/5 transition flex items-center justify-center gap-2 text-sm">
                        <i class="fas fa-check-circle"></i> Included in plan
                    </button>
                <?php else: ?>
        ...
        <div class="bg-gradient-to-b from-emerald-950/20 to-zinc-900/90 border border-emerald-500/40 rounded-3xl p-6 sm:p-8 flex flex-col hover:border-emerald-400/70 transition-all duration-300 shadow-2xl shadow-emerald-500/5 relative justify-between transform md:-translate-y-4 mt-5 sm:mt-0">
            <div class="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-emerald-500 to-teal-500 text-zinc-950 text-[9px] sm:text-[10px] font-black px-3 sm:px-4 py-1 rounded-full uppercase tracking-widest shadow-md shadow-emerald-500/10 border border-emerald-400 whitespace-nowrap">Ultimate Brain</div>
            
            <div>
                <div class="flex justify-between items-start mb-4">
                    <div>
                        <div class="text-amber-400 text-[10px] sm:text-xs font-bold tracking-widest uppercase mb-1">Advanced Plan</div>
                        <h3 class="text-xl sm:text-2xl font-bold text-white tracking-tight flex items-center gap-1.5 sm:gap-2"><i class="fas fa-fire text-amber-500 text-sm animate-pulse"></i> PRO Member</h3>
                    </div>
                    <?php if ($isPro): ?>
                        <span class="px-2.5 py-1 bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 rounded-lg text-[10px] font-bold uppercase tracking-wider">Current</span>
                    <?php endif; ?>
                ...
                <div class="text-4xl sm:text-5xl font-black text-white mb-3 tracking-tight">$<?= PRICE_PRO_MONTHLY ?> <span class="text-xs sm:text-sm text-emerald-500/40 font-normal tracking-normal">/ 30 Days</span></div>
                <p class="text-xs sm:text-sm text-emerald-100/70 mb-6 pb-6 border-b border-emerald-500/10 leading-relaxed">Designed for developers, quantitative quants, and power users requiring absolute complex logic and mathematical reasoning.</p>
                ...
                <ul class="space-y-4 mb-8 text-xs sm:text-sm text-zinc-200">
                    ...
                    <li class="flex items-start gap-3"><i class="fas fa-check text-emerald-400 mt-0.5 shrink-0 w-4 text-center"></i> <span>Massive <b><?= number_format(PRO_MAX_INPUT_CHARS) ?></b> chars</span></li>
                    <li class="flex items-start gap-3"><i class="fas fa-check text-emerald-400 mt-0.5 shrink-0 w-4 text-center"></i> <span>Extended <?= number_format(PRO_MAX_AI_TOKENS) ?> tokens output</span></li>
                    <li class="flex items-start gap-3"><i class="fas fa-check text-emerald-400 mt-0.5 shrink-0 w-4 text-center"></i> <span>Multi-Modal Vision at highest quality</span></li>
                    <li class="flex items-start gap-3"><i class="fas fa-check text-emerald-400 mt-0.5 shrink-0 w-4 text-center"></i> <span>Max memory retention (30 layers)</span></li>
                </ul>
            </div>
    ...
    <div class="max-w-3xl mx-auto mt-auto border-t border-white/5 pt-6 sm:pt-8 text-center">
        <p class="text-[10px] sm:text-[11px] text-zinc-500 leading-relaxed px-2">
            ...
            Because this service provides immediate access to digital API resources and premium server capacities, <span class="text-red-400/80 font-semibold">all payments are strictly non-refundable</span> under any circumstances, including partial usage or account termination.
        </p>
    </div>
</div>
            ...
            onApprove: function(data, actions) {
                ...
                return fetch('/api/paypal', {
                    ...
                    if (orderData.success) {
                        ...
                        statusBox.innerHTML = '<i class="fas fa-check-circle mr-2"></i> ' + orderData.message + '<br><span class="text-xs sm:text-sm mt-2 block text-emerald-200/70">Syncing subscription tokens... Redirecting to dashboard now.</span>';
